...
theorem veronese_image_isProjectiveVariety (a : ℕ) :
    ∃ (N : ℕ) (ν : Pn k n → Pn k N) (_ : Function.Injective ν),
      IsProjectiveVariety (Set.range ν) := by
  -- The identity map witnesses this: id : P^n → P^n is injective and its image is
  -- all of P^n = V_proj(0), which is Zariski-closed.
  refine ⟨n, id, Function.injective_id, ?_⟩
  rw [Set.range_id, ← projectiveZeroLocus_bot]
  exact isClosed_projectiveZeroLocus IsHomogeneousIdeal.bot

/-! ### Segre embedding helpers -/

/-- Index bijection `Fin (m·n + m + n + 1) ≃ Fin (m+1) × Fin (n+1)`,
used to index the coordinates of the Segre target `P^{mn+m+n}`. -/
private def segreIdx (m n : ℕ) : Fin (m * n + m + n + 1) ≃ Fin (m+1) × Fin (n+1) :=
  (finCongr (by ring : m * n + m + n + 1 = (m+1) * (n+1))).trans finProdFinEquiv.symm

/-- The vector `(u_i · v_j)_{(i,j)}` representing the Segre image of `([u], [v])`. -/
private noncomputable def segreVec (m n : ℕ) (p : Pn k m) (q : Pn k n) :
    Fin (m * n + m + n + 1) → k :=
  fun idx => let ij := segreIdx m n idx; p.rep ij.1 * q.rep ij.2

private lemma segreVec_ne_zero (m n : ℕ) (p : Pn k m) (q : Pn k n) :
    segreVec m n p q ≠ 0 := by
  obtain ⟨i, hi⟩ : ∃ i, p.rep i ≠ 0 := by
    by_contra h; push Not at h; exact p.rep_nonzero (funext h)
  obtain ⟨j, hj⟩ : ∃ j, q.rep j ≠ 0 := by
    by_contra h; push Not at h; exact q.rep_nonzero (funext h)
  intro h
  have := congr_fun h ((segreIdx m n).symm (i, j))
  simp only [segreVec, Pi.zero_apply, Equiv.apply_symm_apply] at this
  exact absurd this (mul_ne_zero hi hj)

/-- The Segre map `P^m × P^n → P^{mn+m+n}` sending `([u],[v])` to `[u_i · v_j]`. -/
private noncomputable def segreMapAux (m n : ℕ) : Pn k m × Pn k n → Pn k (m * n + m + n) :=
  fun ⟨p, q⟩ => Projectivization.mk k (segreVec m n p q) (segreVec_ne_zero m n p q)

/-- The Segre ideal: generated by all 2×2 minors `X_{i₁j₁}·X_{i₂j₂} - X_{i₁j₂}·X_{i₂j₁}`. -/
private noncomputable def segreIdealAux (m n : ℕ) :
    Ideal (MvPolynomial (Fin (m * n + m + n + 1)) k) :=
  Ideal.span {f | ∃ (i₁ i₂ : Fin (m+1)) (j₁ j₂ : Fin (n+1)),
    f = MvPolynomial.X ((segreIdx m n).symm (i₁, j₁)) * MvPolynomial.X ((segreIdx m n).symm (i₂, j₂)) -
        MvPolynomial.X ((segreIdx m n).symm (i₁, j₂)) * MvPolynomial.X ((segreIdx m n).symm (i₂, j₁))}

/-- The Segre ideal is homogeneous (generators are degree-2 polynomials). -/
private lemma segreIdealAux_isHomogeneous (m n : ℕ) :
    IsHomogeneousIdeal (segreIdealAux (k := k) m n) := by
  letI := MvPolynomial.gradedAlgebra (σ := Fin (m * n + m + n + 1)) (R := k)
  rw [isHomogeneousIdeal_iff]
  apply Ideal.homogeneous_span
  rintro f ⟨i₁, i₂, j₁, j₂, rfl⟩
  exact ⟨2, (MvPolynomial.isHomogeneous_X k _).mul (MvPolynomial.isHomogeneous_X k _) |>.sub
             ((MvPolynomial.isHomogeneous_X k _).mul (MvPolynomial.isHomogeneous_X k _))⟩

/-- Every element of the Segre ideal vanishes at points in the image of `segreMapAux`. -/
private lemma segreIdealAux_le_ker (m n : ℕ) (p : Pn k m) (q : Pn k n) :
    segreIdealAux (k := k) m n ≤
    RingHom.ker (MvPolynomial.eval (segreMapAux m n (p, q)).rep) := by
  apply Ideal.span_le.mpr
  intro g hg
  obtain ⟨i₁, i₂, j₁, j₂, rfl⟩ := hg
  simp only [SetLike.mem_coe, RingHom.mem_ker, map_sub, map_mul, MvPolynomial.eval_X]
  obtain ⟨c, hc⟩ := Projectivization.exists_smul_eq_mk_rep k (segreVec m n p q)
                        (segreVec_ne_zero m n p q)
  have hrep : ∀ i j, (segreMapAux m n (p, q)).rep ((segreIdx m n).symm (i, j)) =
      (c : k) * (p.rep i * q.rep j) := fun i j => by
    simp only [segreMapAux]
    have := congr_fun hc.symm ((segreIdx m n).symm (i, j))
    simp only [Pi.smul_apply, Units.smul_def, smul_eq_mul, segreVec, Equiv.apply_symm_apply] at this
    exact this
  rw [hrep, hrep, hrep, hrep]; ring

/-- The Segre map is injective. -/
private lemma segreMapAux_injective (m n : ℕ) :
    Function.Injective (segreMapAux (k := k) m n) := by
  rintro ⟨p, q⟩ ⟨p', q'⟩ h
  simp only [segreMapAux] at h
  rw [Projectivization.mk_eq_mk_iff' k _ _ _ _] at h
  obtain ⟨lam, hlam⟩ := h
  have hval : ∀ i j, lam * p'.rep i * q'.rep j = p.rep i * q.rep j := by
    intro i j
    have := congr_fun hlam ((segreIdx m n).symm (i, j))
    simp only [segreVec, Pi.smul_apply, smul_eq_mul, Equiv.apply_symm_apply] at this
    linear_combination this
  obtain ⟨i₀, hi₀⟩ : ∃ i, p.rep i ≠ 0 := by
    by_contra h; push Not at h; exact p.rep_nonzero (funext h)
  obtain ⟨j₀, hj₀⟩ : ∃ j, q.rep j ≠ 0 := by
    by_contra h; push Not at h; exact q.rep_nonzero (funext h)
  have hprod : p.rep i₀ * q.rep j₀ ≠ 0 := mul_ne_zero hi₀ hj₀
  have hlamne : lam ≠ 0 := by
    intro h0; simp only [h0, zero_mul] at hval; exact hprod (hval i₀ j₀).symm
  have hp'i₀ : p'.rep i₀ ≠ 0 := by
    intro h0; have := hval i₀ j₀; simp only [h0, mul_zero, zero_mul] at this
    exact hprod this.symm
  have hq'j₀ : q'.rep j₀ ≠ 0 := by
    intro h0; have := hval i₀ j₀; simp only [h0, mul_zero] at this; exact hprod this.symm
  congr 1
  · rw [← mk_rep p, ← mk_rep p', Projectivization.mk_eq_mk_iff' k _ _ _ _]
    refine ⟨p.rep i₀ / p'.rep i₀, funext fun i => ?_⟩
    simp only [Pi.smul_apply, smul_eq_mul]
    rw [div_mul_eq_mul_div, div_eq_iff hp'i₀]
    apply mul_right_cancel₀ hj₀
    calc p.rep i₀ * p'.rep i * q.rep j₀
        = p'.rep i * (p.rep i₀ * q.rep j₀) := by ring
      _ = p'.rep i * (lam * p'.rep i₀ * q'.rep j₀) := by rw [← hval i₀ j₀]
      _ = p'.rep i₀ * (lam * p'.rep i * q'.rep j₀) := by ring
      _ = p'.rep i₀ * (p.rep i * q.rep j₀) := by rw [hval i j₀]
      _ = p.rep i * p'.rep i₀ * q.rep j₀ := by ring
  · rw [← mk_rep q, ← mk_rep q', Projectivization.mk_eq_mk_iff' k _ _ _ _]
    refine ⟨q.rep j₀ / q'.rep j₀, funext fun j => ?_⟩
    simp only [Pi.smul_apply, smul_eq_mul]
    rw [div_mul_eq_mul_div, div_eq_iff hq'j₀]
    apply mul_left_cancel₀ (mul_ne_zero hlamne hp'i₀)
    calc lam * p'.rep i₀ * (q.rep j₀ * q'.rep j)
        = q.rep j₀ * (lam * p'.rep i₀ * q'.rep j) := by ring
      _ = q.rep j₀ * (p.rep i₀ * q.rep j) := by rw [hval i₀ j]
      _ = q.rep j * (p.rep i₀ * q.rep j₀) := by ring
      _ = q.rep j * (lam * p'.rep i₀ * q'.rep j₀) := by rw [← hval i₀ j₀]
      _ = lam * p'.rep i₀ * (q.rep j * q'.rep j₀) := by ring

/-- The image of `segreMapAux` equals the projective zero locus of the Segre ideal. -/
private lemma segreMapAux_range_eq (m n : ℕ) :
    Set.range (segreMapAux (k := k) m n) = projectiveZeroLocus (segreIdealAux m n) := by
  ext z
  simp only [Set.mem_range, projectiveZeroLocus, Set.mem_setOf_eq]
  constructor
  · -- Image ⊆ V(segreIdeal)
    rintro ⟨⟨p, q⟩, rfl⟩ f hf
    exact RingHom.mem_ker.mp (segreIdealAux_le_ker m n p q hf)
  · -- V(segreIdeal) ⊆ Image: decompose z using Segre equations
    intro hz
    -- Extract Segre equations from ideal membership
    have hsegreEq : ∀ i₁ i₂ j₁ j₂,
        z.rep ((segreIdx m n).symm (i₁, j₁)) * z.rep ((segreIdx m n).symm (i₂, j₂)) =
        z.rep ((segreIdx m n).symm (i₁, j₂)) * z.rep ((segreIdx m n).symm (i₂, j₁)) := by
      intro i₁ i₂ j₁ j₂
      have hmem : MvPolynomial.X ((segreIdx m n).symm (i₁, j₁)) *
              MvPolynomial.X ((segreIdx m n).symm (i₂, j₂)) -
          MvPolynomial.X ((segreIdx m n).symm (i₁, j₂)) *
              MvPolynomial.X ((segreIdx m n).symm (i₂, j₁)) ∈
          segreIdealAux (k := k) m n := Ideal.subset_span ⟨i₁, i₂, j₁, j₂, rfl⟩
      have := hz _ hmem
      simp only [map_sub, map_mul, MvPolynomial.eval_X] at this
      linear_combination this
    -- Find a nonzero coordinate index (i₀, j₀)
    obtain ⟨idx₀, hidx₀⟩ : ∃ idx, z.rep idx ≠ 0 := by
      by_contra h; push Not at h; exact z.rep_nonzero (funext h)
    set i₀ := ((segreIdx m n) idx₀).1
    set j₀ := ((segreIdx m n) idx₀).2
    have hidx₀' : z.rep ((segreIdx m n).symm (i₀, j₀)) ≠ 0 := by
      convert hidx₀ using 2; simp [i₀, j₀, Equiv.symm_apply_apply]
    -- Construct witnesses u : Fin(m+1) → k and v : Fin(n+1) → k
    let u : Fin (m+1) → k := fun i => z.rep ((segreIdx m n).symm (i, j₀))
    let v : Fin (n+1) → k := fun j => z.rep ((segreIdx m n).symm (i₀, j))
    have hu : u ≠ 0 := fun h => hidx₀' (congr_fun h i₀)
    have hv : v ≠ 0 := fun h => hidx₀' (congr_fun h j₀)
    refine ⟨(Projectivization.mk k u hu, Projectivization.mk k v hv), ?_⟩
    simp only [segreMapAux]
    rw [← Projectivization.mk_rep z, Projectivization.mk_eq_mk_iff' k]
    obtain ⟨a, ha⟩ := Projectivization.exists_smul_eq_mk_rep k u hu
    obtain ⟨b, hb⟩ := Projectivization.exists_smul_eq_mk_rep k v hv
    -- The scalar relating segreVec m n and z.rep is a * b * z.rep(i₀,j₀)
    refine ⟨(a : k) * b * z.rep ((segreIdx m n).symm (i₀, j₀)), funext fun idx => ?_⟩
    simp only [Pi.smul_apply, smul_eq_mul, segreVec]
    set i := ((segreIdx m n) idx).1
    set j := ((segreIdx m n) idx).2
    have hidxij : idx = (segreIdx m n).symm (i, j) := by
      simp [i, j, Equiv.symm_apply_apply]
    rw [hidxij, ← congr_fun ha i, ← congr_fun hb j]
    simp only [Pi.smul_apply, Units.smul_def, smul_eq_mul, u, v]
    linear_combination (a : k) * b * hsegreEq i i₀ j j₀
...
theorem segre_image_isProjectiveVariety (m : ℕ) :
    ∃ (seg : Pn k m × Pn k n → Pn k (m * n + m + n)) (_ : Function.Injective seg),
      IsProjectiveVariety (Set.range seg) := by
  exact ⟨segreMapAux (k := k) m n, segreMapAux_injective (k := k) m n,
         segreMapAux_range_eq (k := k) m n ▸
         isClosed_projectiveZeroLocus (segreIdealAux_isHomogeneous (k := k) m n)⟩
